  ...
  try {
    ...
    if (err?.status === 503) {
      try {
        ...
        console.error("[gemini] retry failed:", retryErr?.message ?? retryErr);
        throw new AiError("The AI service is busy. Try again in a moment.");
      }
    } else {
      console.error("[gemini] request failed:", err?.message ?? err);
      throw new AiError("The AI service is unavailable right now.");
    }
  }
